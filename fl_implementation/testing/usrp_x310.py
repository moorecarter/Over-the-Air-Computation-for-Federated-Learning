...
class USRP_X310():
    ...
    @staticmethod
    def grad_to_wave(grads: np.ndarray, amplitude: float, csi: complex = 1.0, sps: int = 4):
        symbols = amplitude * grads.astype(np.complex64) / csi
        upsampled = USRP_X310._upsample(symbols=symbols, sps=sps)
        rrc = USRP_X310.rrc_filter(sps=sps)
        waveform = np.convolve(upsampled, rrc, mode='same').astype(np.complex64)
        return waveform

    @staticmethod
    def wave_to_grad(waveform: np.ndarray, amplitude: float, sps: int = 4):
        rrc = USRP_X310.rrc_filter(sps=sps).astype(np.complex64)
        rx_matched = np.convolve(waveform, rrc, mode='same')
        rx_symbols = rx_matched[::sps]
        grads = rx_symbols / amplitude
