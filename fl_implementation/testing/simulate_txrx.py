...
class OTAChannelModel:
    ...
    def aggregate_with_channel(
        ...
        # Effective weights after channel effects (no renormalization — OTA produces a sum)
...
def create_ota_strategy(
    ...
    return OTAChannelModel(**config)

def test_ota_strategy(client_signals: List[np.ndarray], config_name: str = "ideal"):
    # Test channel model
    ...
    if client_signals is None:
        ...
        client_updates = client_signals

    # Test different channel conditions
    channel = create_ota_strategy(config_name)
    aggregated_result = channel.aggregate_with_channel(client_updates)
